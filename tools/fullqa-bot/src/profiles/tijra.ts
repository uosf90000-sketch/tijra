...
async function json<T = any>(response: { json():Promise<unknown> }) {
...
async function createListing(api:APIRequestContext, target:string, add:AddResult, input:{name:string;sku:string;barcode:string;price:number;quantity:number;minOrderQty:number}) {
  ...
  const body = await json<{listing?:Listing; duplicate?:boolean; error?:string}>(r).catch(()=>({}));
...
async function runLifecycle(target:string, add:AddResult, accounts:Account[], stamp:string) {
  ...
  try {
    ...
    const duplicateBody = await json<{duplicate?:boolean}>(duplicate).catch(()=>({}));
    ...
    const orderBody = await json<{order?:{id:string;status:string;expectedTotal:number|string};orders?:Array<{id:string}>;error?:string}>(orderResponse).catch(()=>({}));
    ...
    for (let i=0;i<listings.length;i++) {
      ...
      const lookupBody = await json<{listing?:Listing}>(lookup).catch(()=>({}));
    ...
    const acceptBody = await json<{order?:{status:string}}>(accept).catch(()=>({}));
    ...
    // Receiving before picking must be blocked. If it succeeds, this is a critical business-flow defect.
    ...
    for (let i=0;i<specs.length;i++) {
      for (let n=1;n<=specs[i].orderQty;n++) {
        ...
        const pickBody = await json<{item?:{scanned:number;required:number};orderComplete?:boolean;error?:string}>(pick).catch(()=>({}));
        scannedTotal++;
    ...
    const receiveBody = await json<{order?:{status:string}}>(receive).catch(()=>({}));
    ...
    const productsBody = await json<{products?:Product[]}>(productsResponse).catch(()=>({}));
    const receivedProducts = specs.map(spec=>productsBody.products?.find(p=>p.barcode===spec.barcode)).filter(Boolean) as Product[];
    ...
    if (receivedProducts[0]) {
      ...
      const saleBody = await json<{sale?:{id:string};duplicate?:boolean;error?:string}>(sale).catch(()=>({}));
      ...
      const duplicateSaleBody = await json<{duplicate?:boolean}>(duplicateSale).catch(()=>({}));
      ...
      const afterBody = await json<{products?:Product[]}>(productsAfterSale).catch(()=>({}));
      const after = afterBody.products?.find(p=>p.id===receivedProducts[0].id);
