...
export function ProductCreateForm({ businessActivity }: { businessActivity: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/products", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ name: form.get("name"), sku: form.get("sku") || undefined, barcode: form.get("barcode") || undefined, imageUrl: form.get("imageUrl") || undefined, category: form.get("category") || undefined, unit: form.get("unit") || "حبة", salePrice: number("salePrice"), averageCost: number("averageCost"), quantity: number("quantity"), reorderPoint: number("reorderPoint"), targetCoverageDays: Math.max(1, Math.round(number("targetCoverageDays") || 7)) }) });
      const result = await response.json().catch(() => ({}));
      if (!response.ok) { setError(result.error === "SKU_ALREADY_EXISTS" ? `${partsBusiness ? "رقم القطعة" : "SKU"} مستخدم لمنتج آخر.` : result.error === "OWNER_REQUIRED" ? "إضافة المنتجات متاحة لمالك المنشأة فقط." : "تعذر إضافة المنتج. راجع البيانات."); return; }
      const productId = result.product?.id;
      router.replace(foodBusiness && productId ? `/recipes?product=${encodeURIComponent(productId)}` : "/products");
      router.refresh();
    } catch {
      setError("تعذر الاتصال بالخادم. لم يتم تأكيد الحفظ؛ حاول مرة أخرى بعد التحقق من الاتصال.");
    } finally {
      setLoading(false);
    }
  }
  ...
  const note = foodBusiness ? "الصورة والاسم والسعر تظهر للكاشير. بعد الحفظ تربط المكونات والإضافات." : partsBusiness ? "رقم القطعة هو أسرع طريقة للكاشير للعثور عليها ومعرفة المتوفر." : electronicsBusiness ? "أضف SKU أو الموديل والباركود. فعّل Serial / IMEI فقط للمنتجات التي تحتاج رقمًا فريدًا." : "أضف البيانات التي يحتاجها البيع والمخزون فقط.";

  return <form className="panel onboardingForm activityProductForm" onSubmit={submit}>
