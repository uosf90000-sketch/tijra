...
import { SupplierShareButton } from "@/components/supplier-share-button";
...
export default async function PurchasesPage() {
  ...
  const suggestions = products.flatMap((product) => {
    ...
    return [{ productId: product.id, product: product.name, current, reorder, suggested, unit: product.unit, supplier: offer.supplier.name, unitPrice: Number(offer.price) }];
  });
  ...
  return (
    <>
      <PageHeader
        ...
        title="المشتريات"
        description="تِجرا يقترح الكمية، يقسم الطلب حسب المورد، ثم تعطي المورد رابطًا يؤكد منه بدون حساب."
        actions={<><Link className="button secondary" href="/purchases/invoice"><Camera size={17} /> مطابقة فاتورة</Link><Link className="button primary" href="/suppliers/prices/new"><CirclePlus size={17} /> إضافة سعر مورد</Link></>}
      />
      ...
      <section className="panel smartOrderPanel">
        <div className="panelHeader"><div><span className="eyebrow"><Sparkles size={14} /> خطة اليوم</span><h2>ماذا نطلب الآن؟</h2></div><span className="aiBadge">Smart replenishment</span></div>
        <div className="smartOrderList">
          {suggestions.map((item) => (
            <div className="smartOrderRow" key={item.productId}>
              <div className="grow"><strong>{item.product}</strong><span>الموجود {item.current} · نقطة الطلب {item.reorder} · {item.supplier}</span></div>
          ...
          {!suggestions.length && <div className="infoNote">لا توجد أصناف منخفضة لها سعر مورد مسجل.</div>}
        </div>
        <div className="orderSummaryBar"><div><span>الإجمالي التقديري</span><strong>{formatSar(suggestionTotal)}</strong></div><SmartOrderButton /></div>
        <p className="policyNote">تِجرا ينظم الطلب والتأكيد فقط. التوصيل وموعده يبقيان مباشرة بين التاجر والمورد.</p>
      </section>
      ...
      <section className="panel tablePanel">
        ...
        <div className="tableScroll">
          <table className="dataTable">
            <thead><tr><th>رقم الطلب</th><th>المورد</th><th>الأصناف</th><th>القيمة</th><th>الحالة</th><th>التاريخ</th><th>إرسال للمورد</th></tr></thead>
            <tbody>
              {purchaseOrders.map((order) => (
                <tr key={order.id}>
                  ...
                  <td>{["RECEIVED", "CANCELLED"].includes(order.status) ? <span className="mutedText">مغلق</span> : <SupplierShareButton orderId={order.id} />}</td>
                </tr>
              ))}
              {!purchaseOrders.length && <tr><td colSpan={7}><div className="infoNote">لا توجد طلبات شراء بعد.</div></td></tr>}
            </tbody>
          </table>
        </div>
      </section>
