...
export default async function MarketplaceSellerPage() {
  ...
  return <>
    ...
    <section className="panel tablePanel" style={{ marginTop: 12 }}><div className="panelHeader tableHeader"><div><span className="eyebrow">مخزون المورد</span><h2>أحدث منتجاتك المعروضة</h2></div><span>{listings.length.toLocaleString("ar-SA")} من {listingCount.toLocaleString("ar-SA")}</span></div><div className="tableScroll"><table className="dataTable"><thead><tr><th>المنتج</th><th>السعر</th><th>المتوفر</th><th>الحد الأدنى</th><th>آخر تحديث</th><th>الحالة</th></tr></thead><tbody>{listings.map((item) => <tr key={item.id}><td><strong>{item.name}</strong>{item.sku && <span className="mutedText" style={{ display: "block" }}>SKU: {item.sku}</span>}{item.barcode && <span className="mutedText" style={{ display: "block" }}>باركود: {item.barcode}</span>}</td><td>{formatSar(Number(item.price))}</td><td>{Number(item.quantity).toLocaleString("ar-SA")} {unitLabel(item.unit)}</td><td>{Number(item.minOrderQty).toLocaleString("ar-SA")} {unitLabel(item.unit)}</td><td><span className="stockUpdatedAt"><Clock3 size={13} /> {stockUpdateFormatter.format(item.updatedAt)}</span></td><td>{item.active ? "معروض" : "متوقف"}</td></tr>)}{!listings.length && <tr><td colSpan={6}><div className="infoNote">أضف أول منتج ليظهر في سوق تِجرا.</div></td></tr>}</tbody></table></div></section>
