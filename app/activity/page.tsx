...
export default async function ActivityPage({ searchParams }: { searchParams: Promise<{ mode?: string }> }) {
  ...
  const [events, orders] = await Promise.all([
    db.inventoryAuditEvent.findMany({ where: { businessId }, orderBy: { occurredAt: "desc" }, take: 120 }),
  ...
  for (const event of events) {
    items.push({
      ...
      href: supplierMode ? "/supplier/stock-count" : "/inventory/audit",
      type: event.action === "CASHIER_SALE" ? "sale" : "stock",
    });
  }
