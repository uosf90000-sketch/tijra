...
export default async function SupplierPickingPage() {
  ...
  const scannedFor = (orderId: string, listingId: string, required: number) => complete.has(orderId)
    ? required
    : Math.min(required, progressMap.get(`${orderId}:${listingId}`) ?? 0);
  const totalRequired = orders.reduce((s, order) => s + order.items.reduce((n, item) => n + Number(item.quantity), 0), 0);
  const totalScanned = orders.reduce((s, order) => s + order.items.reduce((n, item) => n + scannedFor(order.id, item.listingId, Number(item.quantity)), 0), 0);
  return <>
